...
/**
 * @swagger
 * /api/org-setup/nodes/{id}/materials:
 *   get:
 *     summary: Fetch paginated materials for a specific OBS node
 *     parameters:
 *       - in: path
 *         name: id
 *         required: true
...
export async function GET(
  ...
  try {
    ...
    const [materials, total] = await Promise.all([
      ...
      prisma.oBSMaterial.count({ where }),
    ]);

    return NextResponse.json({ materials, total });
  } catch (error) {
    console.error("GET /api/org-setup/nodes/[id]/materials error:", error);
