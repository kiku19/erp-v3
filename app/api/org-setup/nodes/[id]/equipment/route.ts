...
/**
 * @swagger
 * /api/org-setup/nodes/{id}/equipment:
 *   get:
 *     summary: Fetch paginated equipment for a specific OBS node
 *     parameters:
 *       - in: path
 *         name: id
 *         required: true
...
export async function GET(
  ...
  try {
    ...
    const [equipment, total] = await Promise.all([
      ...
      prisma.oBSEquipment.count({ where }),
    ]);

    return NextResponse.json({ equipment, total });
  } catch (error) {
    console.error("GET /api/org-setup/nodes/[id]/equipment error:", error);
