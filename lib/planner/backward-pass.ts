  ...
  while (queue.length > 0) {
    ...
    if (!succEntries || succEntries.length === 0) {
      ...
      for (const entry of succEntries) {
        const ms = entry.lateStart.getTime() - entry.lag * MS_PER_DAY;
    ...
    lateStart.setUTCDate(lateStart.getUTCDate() - duration);

    // Total float = late start - early start (in days)
    const fp = forwardResults.get(actId);
    const earlyStartMs = fp ? new Date(fp.startDate).getTime() : 0;
    const totalFloat = Math.round((lateStart.getTime() - earlyStartMs) / MS_PER_DAY);
