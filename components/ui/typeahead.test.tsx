...
describe("Typeahead", () => {
  ...
  it("calls onChange when an item is clicked", async () => {
    ...
    render(<Typeahead items={items} onChange={onChange} />);
  ...
  it("navigates suggestions with arrow keys", async () => {
    ...
    expect(options[0].className).not.toContain("bg-accent");
  });
  ...
  it("renders item icons", () => {
    render(<Typeahead items={items} />);
    // Icons are rendered as SVGs inside the items
    const options = screen.getAllByRole("option");
  ...
  it("wraps arrow navigation at boundaries", async () => {
    ...
    const options = screen.getAllByRole("option");
    expect(options[options.length - 1].className).toContain("bg-accent");
  });

  it("merges custom className", () => {
    render(<Typeahead items={items} className="my-custom" />);
    const container = screen.getByRole("listbox").closest("[class*='my-custom']");
